...
export default function LearnSortable({
  ...
  const renderChip = (
    ...
    const bucketIncorrect = showValidation && validation.incorrectBuckets.includes(item.text as 'A' | 'B');
    return (
      <TouchableOpacity
        key={item.text}
        ...
        <View className="flex-row gap-4" style={{ flex: 1 }}>
          {buckets.map((bucket) => {
            ...
            return (
              <View
                key={bucket.id}
                className="min-h-[140px] flex-1 flex-col rounded-lg border-2 border-dashed p-3"
                style={{
